...
const (
	...
	IrysPrivateKeyFlagName     = "irys.private-key"
	IrysPaymentNetworkFlagName = "irys.network"
	IrysPaymentRpcFlagName     = "irys.rpc"
	IrysFreeUploadFlagName     = "irys.free-upload"
)
...
var (
	...
	IrysPrivateKeyFlag = &cli.StringFlag{
		Name:    IrysPrivateKeyFlagName,
		Usage:   "private key for irys",
		Value:   "",
		EnvVars: prefixEnvVars("IRYS_PRIVATE_KEY"),
	}
	IrysPaymentNetworkFlag = &cli.StringFlag{
		Name:    IrysPaymentNetworkFlagName,
		Usage:   "payment network for irys, may be ethereum,matic,bnb,avalanche,arbitrum or fantom",
		Value:   "",
		EnvVars: prefixEnvVars("IRYS_PAYMENT_NETWORK"),
	}
	IrysPaymentRpcFlag = &cli.StringFlag{
		Name:    IrysPaymentRpcFlagName,
		Usage:   "rpc for irys payment network",
		Value:   "",
		EnvVars: prefixEnvVars("IRYS_PAYMENT_RPC"),
	}
	IrysFreeUploadFlag = &cli.BoolFlag{
		Name:    IrysFreeUploadFlagName,
		Usage:   "irys uploads of less than 100 KiB are free, default is true",
		Value:   true,
		EnvVars: prefixEnvVars("IRYS_FREE_UPLOAD"),
	}
)
...
type CLIConfig struct {
	...
	IrysPaymentNetwork string
	IrysPaymentRpc     string
	IrysFreeUpload     bool
}
...
func ReadCLIConfig(ctx *cli.Context) CLIConfig {
	return CLIConfig{
		...
		IrysPrivateKey:     ctx.String(IrysPrivateKeyFlagName),
		IrysPaymentNetwork: ctx.String(IrysPaymentNetworkFlagName),
		IrysPaymentRpc:     ctx.String(IrysPaymentRpcFlagName),
		IrysFreeUpload:     ctx.Bool(IrysFreeUploadFlagName),
	}
}

func (c CLIConfig) Check() error {
	if !c.S3Enabled() && !c.FileStoreEnabled() && !c.IrysEnabled() {
...
func (c CLIConfig) IrysConfig() opplasmairys.Config {
	return opplasmairys.Config{
		NetworkName: c.IrysPaymentNetwork,
		PrivateKey:  c.IrysPrivateKey,
		NetWorkRpc:  c.IrysPaymentRpc,
		FreeUpload:  c.IrysFreeUpload,
	}
}

func (c CLIConfig) IrysEnabled() bool {
	return !(c.IrysPaymentNetwork == "" && c.IrysPrivateKey == "")
}
